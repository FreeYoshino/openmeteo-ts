/**
 * Open-Meteo Weather Forecast API — Weather Variable Definitions
 *
 * Defines the literal union types for all weather variables available
 * via the Open-Meteo API (hourly, daily, current, and minutely_15 intervals).
 *
 * These types serve as the single source of truth shared between:
 * - Query parameter types (query.ts) — what the user can request
 * - Response types (response.ts) — what the API can return
 *
